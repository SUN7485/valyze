...
async def _html_to_pdf_gotenberg(html_content: str, report_id: str) -> Optional[Path]:
    """
    ...
    try:
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"[PDF] OK Gotenberg generated {output_path} "
                    f"({len(response.content) // 1024} KB)"
                )
                return output_path
            else:
                print(
                    f"[PDF] ERROR Gotenberg error {response.status_code}: "
                    f"{response.text[:200]}"
                )
                return None
        ...
        print(
            f"[PDF] ERROR Cannot connect to Gotenberg at {GOTENBERG_URL}. "
            f"Is Docker running?"
        )
        return None
...
async def download_report_pdf(report_id: str):
    ...
    if not pdf_path.exists():
        raise HTTPException(
            404,
            "PDF not found. Generate it first via POST /api/pdf/generate/{report_id}",
        )
    ...
    if report:
        ...
        if f:
            v = (
                f.value
                if hasattr(f, "value")
                else (f.get("value") if isinstance(f, dict) else None)
            )
            if v:
                company_name = v
...
async def preview_report_html(report_id: str):
    ...
    try:
        ...
        if not html_content or len(html_content) < 100:
            print(
                f"[PDF] WARNING: Preview HTML too short ({len(html_content) if html_content else 0} chars)"
            )
            return HTMLResponse(
                content=f"<html><body><h1>Preview Error</h1><p>Generated HTML is too short. Report data may be empty.</p><p>Company: {report.fields.get('company_name', {}).get('value', 'N/A')}</p></body></html>"
            )
        ...
        traceback.print_exc()
        return HTMLResponse(
            content=f"<html><body><h1>Preview Error</h1><p>Error: {str(e)}</p></body></html>"
        )
...
async def get_pdf_status(report_id: str):
    ...
    return {
        ...
        "pdf_exists": exists,
        "pdf_size_kb": round(pdf_path.stat().st_size / 1024, 1) if exists else None,
