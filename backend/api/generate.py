...
router = APIRouter(
    prefix="/api/generate", tags=["generate"], dependencies=[Depends(get_current_user)]
)
...
def should_overwrite_field(report, field_name: str) -> bool:
    ...
    if source == "easy_way_import":
        print(
            f"[GENERATE] SKIP {field_name} - source is 'easy_way_import', preserving imported value"
        )
...
async def generate_narratives(report_id: str):
    """
    ...
    # 3. Set status
    await update_report_status(None, report_id, "generating")
    ...
    try:
        ...
        # 6. Run Generation
        # Narratives
        narrative_results = await narrative_gen.generate_all_narratives(
        ...
        if has_existing_swot:
            print(
                f"[GENERATE] SWOT arrays already populated from JSON - preserving existing data"
            )
            swot_result = {
        ...
        if arrays.get("recommendations") and len(arrays.get("recommendations", [])) > 0:
            print(
                f"[GENERATE] Recommendations already populated from JSON - preserving"
            )
        ...
        if (
            arrays.get("risk_mitigations")
            and len(arrays.get("risk_mitigations", [])) > 0
        ):
            print(
                f"[GENERATE] Risk mitigations already populated from JSON - preserving"
            )
        ...
        monitoring = None
        if (
            arrays.get("monitoring_triggers")
            and len(arrays.get("monitoring_triggers", [])) > 0
        ):
            print(
                f"[GENERATE] Monitoring triggers already populated from JSON - preserving"
            )
        ...
        await update_report_status(None, report_id, "complete")
...
async def get_generation_progress(report_id: str):
    ...
    if db_status == "generating":
        if (
            progress_mtime == 0 or (current_time - progress_mtime) > 300
        ):  # 5 minutes timeout
            is_stuck = True
            file_status = "stuck"
    ...
    is_complete = (
        file_status in ["done", "complete", "ready", "stuck", "finished"]
        or progress_percent >= 100
        or db_status in ["done", "complete", "ready", "finished"]
    )
...
async def regenerate_sections(
    ...
    for section in sections:
        ...
        if section == "swot":
            ...
            if (
                arrays.get("recommendations")
                and len(arrays.get("recommendations", [])) > 0
            ):
                print(
                    "[REGENERATE] Recommendations already populated from JSON - skipping"
                )
                continue
            ...
            if (
                ...
                print(
                    "[REGENERATE] Risk mitigations already populated from JSON - skipping"
                )
                continue
            ...
            if (
                ...
                print(
                    "[REGENERATE] Monitoring triggers already populated from JSON - skipping"
                )
                continue
            ...
            if not should_overwrite_field(report, section):
                print(
                    f"[REGENERATE] Field {section} is from easy_way_import - skipping"
                )
                continue
            ...
            if field_data:
                ...
                if (
                    val
                    and str(val).strip()
                    and str(val).lower() not in ["n/a", "none", "", "null", "pending"]
                ):
                    print(
                        f"[REGENERATE] Field {section} already populated from JSON - skipping"
                    )
                    continue
