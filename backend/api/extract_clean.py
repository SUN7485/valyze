...
async def start_extraction(
        ...
        try:
            ...
            if tables:
                ...
                for arr_key in ("shareholders", "banking_relationships", "branches"):
                    ...
                    if arr_items:
                        all_array_data[arr_key].extend(arr_items)
    ...
    print(f"[CALC] Running calculation engine...")
    ...
    try:
        ...
        # 7a. Financial ratios
        calc_ratios = ratios_engine.calculate_all(report.fields)
        raw_ratios = {}  # Store raw numeric values for scoring engine

        for k, v in calc_ratios.items():
                ...
                if k in report.fields:
                    ...
                    if getattr(existing, 'source', '') == "easy_way_import":
                        ...
                        if raw_val is not None:
                            raw_ratios[k] = raw_val
                        continue
                    report.fields[k].value = v.get("value")
                    report.fields[k].confidence = v.get("confidence", "calculated")
                    report.fields[k].source = v.get("source", "calculated")
                    report.fields[k].locked = v.get("locked", True)
                # Save raw numeric value for scoring
                raw_val = _safe_float(v.get("value"))
                if raw_val is not None:
                    raw_ratios[k] = raw_val
        ...
        print(f"[CALC] Ratios calculated: {list(raw_ratios.keys())}")
        ...
        if "financial_trend" in report.fields:
            ...
            if getattr(existing, 'source', '') != "easy_way_import":
                ...
                print("[CALC] Skipping financial_trend - from Easy Way Import")
        ...
        for k, v in calc_scores.items():
            if k in report.fields:
                ...
                # Skip if field came from Easy Way Import
                if existing.source == "easy_way_import":
                    print(f"[CALC] Skipping {k} - from Easy Way Import")
                    continue
                # Skip if field was user-edited
                if existing.source == "user":
                    print(f"[CALC] Skipping {k} - user-edited")
                    continue
                # Skip if already calculated (avoid double-calculation)
                if existing.confidence == "calculated":
        ...
        print(f"[CALC] Scoring complete")

    except Exception as exc:
        print(f"[CALC] Calculation error (non-fatal): {exc}")
        traceback.print_exc()
